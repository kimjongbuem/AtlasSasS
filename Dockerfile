...
WORKDIR /src
COPY ["AtlasSasS.csproj", "."]
RUN dotnet restore "./AtlasSasS.csproj"
COPY . .
WORKDIR "/src/."
RUN dotnet build "./AtlasSasS.csproj" -c %BUILD_CONFIGURATION% -o /app/build

# 이 스테이지는 최종 스테이지에 복사할 서비스 프로젝트를 게시하는 데 사용됩니다.
FROM build AS publish
ARG BUILD_CONFIGURATION=Release
RUN dotnet publish "./AtlasSasS.csproj" -c %BUILD_CONFIGURATION% -o /app/publish /p:UseAppHost=false

# 이 스테이지는 프로덕션에서 사용되거나 VS에서 일반 모드로 실행할 때 사용됩니다(디버그 구성을 사용하지 않는 경우 기본값).
FROM base AS final
WORKDIR /app
COPY --from=publish /app/publish .
ENTRYPOINT ["dotnet", "AtlasSasS.dll"]